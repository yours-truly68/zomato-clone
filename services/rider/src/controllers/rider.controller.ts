...
export const getRiderProfile = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    const user = req.user;
    if (!user) {
      return res.status(401).json({ message: "Unauthorized - User not found" });
...
export const acceptOrderByRider = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const { data } = await axios.put(
        `${process.env.RESTAURANT_URL}/api/order/assign/rider`,
        {
          orderId,
          riderId: rider._id,
...
export const updateOrderStatus = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    try {
      const { data } = await axios.put(
        `${process.env.RESTAURANT_URL}/api/order/update/rider/status`,
        {
          orderId,
          riderId: rider._id,
        },
